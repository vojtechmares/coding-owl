...
func TestRebasePutsAWorktreeBackWhenTheAbortItselfRefuses(t *testing.T) {
	dir := newRepo(t)
	commit(t, dir, ".gitattributes", "*.owl filter=owlstop\n")
	worktree := filepath.Join(t.TempDir(), "job")
	if err := git.AddWorktree(dir, worktree, "owl/job-1", "main"); err != nil {
		t.Fatalf("AddWorktree: %v", err)
	}
	// Three commits on the branch: one adding a filtered file, one taking it
	// away, and one adding another filtered file that stays. Replaying the
	// first runs the filter and stops there; aborting puts the branch's head
	// back, which runs the filter again over the file that stays.
	if err := os.WriteFile(filepath.Join(worktree, "first.owl"), []byte("replayed\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	run(t, worktree, "add", "--", "first.owl")
	run(t, worktree, "commit", "-m", "a commit the filter is run over while replaying")
	run(t, worktree, "rm", "-q", "--", "first.owl")
	run(t, worktree, "commit", "-m", "and take it away again")
	if err := os.WriteFile(filepath.Join(worktree, "kept.owl"), []byte("kept\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	run(t, worktree, "add", "--", "kept.owl")
	run(t, worktree, "commit", "-m", "a filtered file that stays")
	before := strings.TrimSpace(run(t, worktree, "rev-parse", "HEAD"))
	commit(t, dir, "from-base.txt", "moved on\n")
	// The filter refuses the first two times git runs it - the replay, and
	// then the abort's own checkout - and passes everything after. So the
	// abort refuses, deterministically, and only what comes after it can
	...
	// Reported as what it was: an abort that refused, and a branch put back
	// by force, rather than a rebase that merely would not go through.
	if !strings.Contains(err.Error(), "put back by force") {
		t.Errorf("the error does not say the branch was put back by force: %v", err)
	}
...
// refusing makes a repository require a smudge filter that refuses the first
// n times git runs it and passes everything through after that. Only the
// smudge side counts: the clean side is run whenever git compares a worktree
// with its index, which would make the count depend on what git felt like
// checking.
func refusing(t *testing.T, dir string, n int) {
	t.Helper()
	...
	script := "#!/bin/sh\n" +
		...
		"n=$((n+1)); echo $n > " + count + "\n" +
		"if [ $n -le " + strconv.Itoa(n) + " ]; then cat > /dev/null; exit 1; fi\n" +
		"cat\n"
	if err := os.WriteFile(path, []byte(script), 0o755); err != nil {
		t.Fatal(err)
	}
	run(t, dir, "config", "filter.owlstop.clean", "cat")
